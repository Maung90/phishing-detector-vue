  ...
  <div class="min-h-screen bg-primary pb-10">
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 class="text-3xl font-bold text-white mb-2">Kunci API</h1>
          <p class="text-white/60">Kelola kunci akses untuk integrasi aplikasi Anda.</p>
        </div>
      ...
      <div class="glass-effect rounded-2xl shadow-2xl overflow-hidden bg-[#2c63d1]/5 border border-[#2c63d1]/10">
        <div v-if="apiStore.isLoading && apiStore.apiKeys.length === 0" class="p-10 text-center text-white/60">
          Memuat kunci API...
        </div>
        ...
        <table v-else class="w-full">
          <thead class="bg-[#2c63d1]/20 border-b border-white/10">
            <tr>
              <th class="px-6 py-4 text-left text-xs font-semibold text-white/60 uppercase tracking-wider">Nama</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-white/60 uppercase tracking-wider">Prefix Kunci</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-white/60 uppercase tracking-wider">Dibuat Pada</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-white/10">
            <tr v-for="key in apiStore.apiKeys" :key="key.id" class="hover:bg-white/5 transition-colors">
              ...
              <td class="px-6 py-4">
                <code class="text-sm font-mono text-[#eded74] bg-black/20 px-2 py-1 rounded">
                  {{ key.key_prefix }}••••••••••••••••
                </code>
              </td>
        ...
        <div v-if="apiStore.apiKeys.length === 0 && !apiStore.isLoading" class="p-10 text-center">
          <Key class="w-12 h-12 text-white/40 mx-auto mb-3" />
          <h3 class="text-lg font-bold text-white">Belum Ada Kunci API</h3>
          <p class="text-white/60">Buat kunci pertama Anda untuk mulai menggunakan layanan SEQR.</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-[#1e2530] rounded-2xl p-8 max-w-md w-full border border-white/10 shadow-2xl">
        <h3 class="text-2xl font-bold text-white mb-6">Buat Kunci API Baru</h3>
        <form @submit.prevent="handleCreateKey">
          <div class="mb-6">
            <label class="block text-sm font-medium text-white/80 mb-2">Nama Kunci</label>
            <input
              v-model="newKeyName"
              type="text"
              required
          ...
          <div class="flex gap-3">
            <button type="button" @click="showCreateModal = false" class="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-lg">
              Batal
            </button>
            <button type="submit" :disabled="isCreating" class="flex-1 py-3 bg-[#2c63d1] hover:bg-[#2c63d1]/80 text-white font-bold rounded-lg disabled:opacity-50">
    ...
    <div v-if="showSecretModal" class="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div class="bg-[#1e2530] rounded-2xl p-8 max-w-lg w-full border border-green-500/30 shadow-2xl">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="text-2xl font-bold text-white">Kunci API Berhasil Dibuat!</h3>
        </div>
        ...
        <button @click="showSecretModal = false" class="w-full py-3 bg-white/10 hover:bg-white/20 text-white font-bold rounded-lg transition-all">
...
import { Header, Navbar } from './components' // Sesuaikan path komponenmu
